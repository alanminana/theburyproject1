...
(() => {
    ...
    // Tras guardar se recarga en vez de parchar el DOM: las filas/tarjetas armadas en el cliente no
    // traían "Eliminar" ni el RowVersion vigente, y los contadores (KPIs, chips "Todas/Disponibles/En uso")
    // quedaban desactualizados hasta el próximo F5.
    function onCajaGuardada(entity, form) {
        if (!entity) return;

        const isEdit = form && form.querySelector('[name="Id"]') && form.querySelector('[name="Id"]').value !== '0';
        showPageFeedback((isEdit ? 'Caja actualizada: ' : 'Caja creada: ') + entity.nombre, 'success');
